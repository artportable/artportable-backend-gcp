...
namespace Artportable.ImageApi.Tests
{
    public class ArtworkServiceTests
    {
        ...
        public ArtworkServiceTests()
        {
            ...
            _mapperMock
                .Setup(m => m.Map<ArtworkDTO>(It.IsAny<Artwork>()))
                .Returns(GetMapperReturnValue());
        }
        ...
        public void CreateArtworkTest()
        {
            ...
            var expected = new ArtworkDTO()
            {
                ...
                Width = 70,
                Depth = 30,
                PrimaryFile = new FileDTO { Name = "a4d7cea7-1831-4fda-8267-4b395f8efc92.jpg" },
                SecondaryFile = null,
                TertiaryFile = null,
                Tags = new List<string>(),
                Title = "Ett nytt konstverk",
            ...
            act.Should()
                .BeEquivalentTo(
                    expected,
                    options => options.Excluding(o => o.Id).Excluding(o => o.Published)
                );
        }
        ...
        public void GetTagsForArtworkTest()
        {
            ...
            var artworkId = new Guid("19459c8f-9b88-4c8f-b314-fb8cec67e225");
            var artworkService = new ArtworkService(_contextMock.Object, _mapperMock.Object);
            var expected = new List<TagDTO> { new TagDTO { Tag = "summer" } };
            ...
            return new List<Artwork>
            {
                new Artwork
                {
                    ...
                    User = new User
                    {
                        Username = "knatte",
                        SocialId = new Guid("5185fe67-a161-468f-ae16-575972553509"),
                        File = new File { Name = "873c48f4-c353-4c75-b05d-f11167682187.jpg" },
