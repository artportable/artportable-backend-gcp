...
namespace Artportable.API.Services
{
    public class UserService : IUserService
    {
        ...
        {
            var user = _context
                .Users.Include(x => x.UserProfile)
                .Join(
                    _context.Subscriptions,
                    x => x.SubscriptionId,
                    y => y.User.SubscriptionId,
                    (x, y) => new { User = x, Subscription = y }
                )
                .Where(x => x.User.Subscription.ProductId != 1)
                .Select(x => new UserDTO()
                {
                    Username = x.User.Username,
                    Surname = x.User.UserProfile.Surname,
                    Name = x.User.UserProfile.Name,
                })
                .ToList();
            return user;
        }
        ...
        {
            var users = _context.Users.Include(u => u.File).Where(u => u.Username.Contains(q));
        ...
        public ProfileDTO GetProfile(string username, string myUsername)
        {
            ...
            if (myUsername != null)
            {
                dto.FollowedByMe = _context.Connections.Any(c =>
                    c.Followee.Username == username && c.Follower.Username == myUsername
                );
            }
        ...
        public ProfileDTO UpdateProfile(string username, UpdateProfileDTO updatedProfile)
        {
            ...
            setSafely(
                ...
                val =>
                {
                    rowToUpdate.About = val;
                }
            );
            setSafely(
                updatedProfile.InspiredBy,
                val =>
                {
                    rowToUpdate.InspiredBy = val;
                }
            );
            setSafely(
                updatedProfile.Studio?.Text,
                val =>
                {
                    rowToUpdate.StudioText = val;
                }
            );
            setSafely(
                updatedProfile.Studio?.Location,
                val =>
                {
                    rowToUpdate.StudioLocation = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.Website,
                val =>
                {
                    rowToUpdate.Website = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.Instagram,
                val =>
                {
                    rowToUpdate.InstagramUrl = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.Facebook,
                val =>
                {
                    rowToUpdate.FacebookUrl = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.LinkedIn,
                val =>
                {
                    rowToUpdate.LinkedInUrl = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.Behance,
                val =>
                {
                    rowToUpdate.BehanceUrl = val;
                }
            );
            setSafely(
                updatedProfile.SocialMedia?.Dribble,
                val =>
                {
                    rowToUpdate.DribbleUrl = val;
                }
            );
            setSafely(
                updatedProfile.ChosenColor,
                val =>
                {
                    rowToUpdate.ChosenColor = val;
                }
            );
            setSafely(
                updatedProfile.ChosenCorners,
                val =>
                {
                    rowToUpdate.ChosenCorners = val;
                }
            );
            setSafely(
                updatedProfile.ChosenFont,
                val =>
                {
                    rowToUpdate.ChosenFont = val;
                }
            );
            setSafely(
                updatedProfile.ChosenFrame,
                val =>
                {
                    rowToUpdate.ChosenFrame = val;
                }
            );
            setSafely(
                updatedProfile.ChosenLayout,
                val =>
                {
                    rowToUpdate.ChosenLayout = val;
                }
            );
            setSafely(
                updatedProfile.ChosenShadow,
                val =>
                {
                    rowToUpdate.ChosenShadow = val;
                }
            );
            setSafely(
                updatedProfile.EmailInformedFollowersDate,
                val => rowToUpdate.User.EmailInformedFollowersDate = val
            );

            if (updatedProfile.EmailDeclinedArtworkUpload.HasValue)
            {
                rowToUpdate.User.EmailDeclinedArtworkUpload = updatedProfile
            ...
            {
                rowToUpdate.Educations = updatedProfile
            ...
            {
                rowToUpdate.Exhibitions = updatedProfile
                    .Exhibitions.Select(e => new Exhibition()
                    {
                        Name = e.Name,
                        Place = e.Place,
                        From = e.From,
        ...
        public List<SimilarProfileDTO> GetSimilarProfiles(string username)
        {
            ...
            var tagsUsedByUser = _context
                .Tags.Include(t => t.Artworks)
            ...
            if (!tagsUsedByUser.Any())
            {
                return _context
                    ...
                    .Select(u => new SimilarProfileDTO
                    {
                        ...
                        ProfilePicture = u.File.Name,
                        Artworks = _context
                            .Artworks.Include(a => a.PrimaryFile)
        ...
        {
            return _context
                .Tags.Where(t => t.Artworks.Any(a => a.User.Username == username))
        ...
        public void UpdateProfilePicture(string filename, string username)
        {
            ...
            var file = _context.Files.SingleOrDefault(f => f.Name == filename);
        ...
        {
            var user = _context
                .Users.Include(u => u.CoverPhotoFile)
                .SingleOrDefault(u => u.Username == username);
