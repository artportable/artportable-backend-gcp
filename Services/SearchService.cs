    ...
    public class SearchService : ISearchService
    {
        ...
        )
        {
            try
            {
                ...
                Console.WriteLine($"Search request - Query: {q}, Page: {page}, Tags: {string.Join(",", tags ?? new List<string>())}");
                ...
                if (tags != null && tags.Any())
                {
                    ...
                    query = query.Where(a => a.Tags.Any(t => normalizedTags.Contains(t.Title.ToLower())));
                }
                ...
                Console.WriteLine($"Search completed successfully. Found {results.Count} results.");
                return results;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error in SearchArtworks: {ex.Message}");
