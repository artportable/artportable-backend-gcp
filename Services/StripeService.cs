...
namespace Artportable.API.Services
{
    public class StripeService : IStripeService
    {
        ...
        public StripeService(
            APContext apContext,
            IOptions<ProductCodes> productCodes,
        ...
        public void HandleEvent(Event e)
        {
            ...
            switch (e.Type)
            {
                ...
                case (Events.InvoicePaid):
                    ...
                    var stripeProduct = _products.FirstOrDefault(p =>
                        p.Key == invoice.Lines.Data[0].Price.ProductId
                    );
                        ...
                        _ = Task.Run(async () =>
                        {
                            try
                            {
                                await _crmService.RegisterPurchase(
                                    ...
                                    (
                                        Convert.ToDecimal(
                                            invoice.Lines.Data[0].Price.UnitAmount / 100
                                        )
                                    ),
                                    invoice.Lines.Data[0].Price.Currency.ToUpper(),
                                    Enum.TryParse<PaymentIntervalEnum>(
                                        invoice.Lines.Data[0].Plan.Interval,
                                        true,
                                        out var interval
                                    )
                                        ? interval
                                        : PaymentIntervalEnum.Month
                                );
                            }
                ...
                case (Events.CustomerSubscriptionUpdated):
                    ...
                    product = _products
                        .FirstOrDefault(p => p.Key == subscription.Items.Data[0].Price.ProductId)
                        .Value;
                    SetSubscription(
