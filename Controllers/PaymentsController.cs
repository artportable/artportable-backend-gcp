...
namespace Artportable.API.Controllers
{
  ...
  public class PaymentsController : ControllerBase
  {
    ...
    public IActionResult GetPromotion(string promotionCode)
    {
      ...
      [HttpPost("purchase/portfolioBoost")]
      public async Task<IActionResult> PurchasePortfolioBoost([FromBody] PurchaseRequest request)
      {
          if (request == null || string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.FullName) ||
              string.IsNullOrWhiteSpace(request.PaymentMethodId) || string.IsNullOrWhiteSpace(request.ProductKey))
          {
          ...
          try
          {
              string customerId = await _paymentService.CreateRocketCustomer(request.Email, request.FullName, request.PhoneNumber);
              string priceId = DeterminePriceId(request.ProductKey);
              ...
              var paymentIntent = await _paymentService.CreatePurchase(customerId, request.PaymentMethodId, "price_1OlVwFA3UXZjjLWxmY3dSDqT");
              return Ok(new { PaymentIntentId = paymentIntent.Id });
          }
          catch (Exception ex)
          {
              return StatusCode(500, "An error occurred while processing your payment: " + ex.Message);
          }
      }


    private string DeterminePriceId(string productKey)
    {
        var productPriceMappings = new Dictionary<string, string>
        {
            { "portfolioBoost", "price_1OlVwFA3UXZjjLWxmY3dSDqT" },
        };

        if (productPriceMappings.TryGetValue(productKey, out var priceId))
        {
            return priceId;
        }
